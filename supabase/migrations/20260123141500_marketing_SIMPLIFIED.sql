-- ================================================
-- Migration: Sistema Completo de Marketing (VERSÃO SIMPLIFICADA)
-- Description: Adiciona agendamento, aniversariantes e envio em massa
-- POLÍTICAS RLS SIMPLIFICADAS - SEM CHECAGEM DE ROLE
-- ================================================
...
-- 7. Políticas RLS SIMPLIFICADAS (SEM CHECAGEM DE ROLE)
ALTER TABLE scheduled_campaigns ENABLE ROW LEVEL SECURITY;

-- REMOVER políticas existentes
DROP POLICY IF EXISTS "Permitir visualização de campanhas agendadas" ON scheduled_campaigns;
DROP POLICY IF EXISTS "Permitir criação de campanhas agendadas para managers" ON scheduled_campaigns;
DROP POLICY IF EXISTS "Permitir atualização de campanhas agendadas" ON scheduled_campaigns;
DROP POLICY IF EXISTS "Permitir exclusão de campanhas agendadas" ON scheduled_campaigns;

-- Políticas SIMPLIFICADAS - Permitir tudo para usuários autenticados
CREATE POLICY "Permitir visualização de campanhas agendadas"
    ON scheduled_campaigns
    FOR SELECT
    TO authenticated
    USING (true);

CREATE POLICY "Permitir criação de campanhas agendadas"
    ON scheduled_campaigns
    FOR INSERT
    TO authenticated
    WITH CHECK (true);

CREATE POLICY "Permitir atualização de campanhas agendadas"
    ON scheduled_campaigns
    FOR UPDATE
    TO authenticated
    USING (true);

CREATE POLICY "Permitir exclusão de campanhas agendadas"
    ON scheduled_campaigns
    FOR DELETE
    TO authenticated
    USING (true);
...
DROP TRIGGER IF EXISTS trigger_update_executed_at ON scheduled_campaigns;
...
-- 9. Garantir permissões adicionais
