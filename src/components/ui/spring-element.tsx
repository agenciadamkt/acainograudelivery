...
const SpringElement = React.forwardRef<HTMLDivElement, SpringAvatarProps>(
  (
    ...
    // The spring values follow x and y but with physics
    ...
    // Update the anchor point only when NOT dragging
    React.useLayoutEffect(() => {
      function update() {
        if (childRef.current && !isDragging) {
    ...
    return (
      <>
        <svg
          className="fixed inset-0 w-screen h-screen pointer-events-none z-40 overflow-visible"
        >
          <path
            d={path}
            strokeLinecap="round"
        ...
        <motion.div
          ...
          style={{ x, y }} // Element directly follows mouse
          drag
